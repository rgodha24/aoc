...
const getData = (input: string, type: "example" | "real") => part1postprocess(parseData(input, type), type);
...
describe("test part 1", () => {
  ...
  it("returns the correct value for the example", () => {
    const type = "example";
    expect(String(part1solver(getData(part1example, type), type))).toBe(part1exampleanswer);
  });
  it("returns a value for normal input", () => {
    const type = "real";
    const answer = part1solver(getData(part1, type), type);
    console.log(answer);
