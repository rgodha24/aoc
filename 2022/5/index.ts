...
export function part1(file: string) {
  ...
  const crates = utils
    .rowsToColumns(
      input[0]
        .split("\n")
        .map((line) => line.replaceAll("[", "").replaceAll("]", "").replaceAll("   ", "").split(" "))
        .slice(0, -1)
        .map((line, index, array) => {
          if (line.length !== array[array.length - 1].length) {
            for (let index = line.length; index < array[array.length - 1].length; index++) {
              line.push("");
            }
          }
          return line;
  ...
  const instructions = input[1]
    ...
    .map(
      (line) =>
        line.replace("move", "").replace(" from", "").replace(" to", "").split(" ").map(Number).slice(1, 4) as [
          number,
          number,
          number
        ]
  ...
  instructions.forEach((instruction) => {
    const data = new Array(instruction.amount).fill("").map(() => crates[instruction.from - 1].shift() as string);
