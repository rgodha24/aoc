...
async function main() {
  ...
  const pnpmWorkspace = parse(readFileSync(a, "utf-8"))
    .packages as Array<unknown>;
  ...
  const files = glob
    ...
    .map(
      (x) => [x, x.replace("template", `${year}/${day}`)] as [string, string]
    );

  mkdirSync(path.join(__dirname, `../${year}/${day}`), { recursive: true });
  mkdirSync(path.join(__dirname, `../${year}/${day}/inputs`), {
    recursive: true,
  });
  ...
  const filesToUpdate = [
    readFileSync(
      path.join(__dirname, `../${year}/${day}/package.json`),
      "utf-8"
    ),
  ...
  filesToUpdate.forEach((file, index) => {
    ...
    writeFileSync(
      path.join(
        __dirname,
        `../${year}/${day}/${index === 0 ? "package.json" : "README.md"}`
      ),
      b
    );
  });
